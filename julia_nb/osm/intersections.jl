...
function segmentHighways(nodes, highways::Dict{Int,Highway}, intersections, classes, levels=Set(1:6))
    segments = Segment[]
        ...
        if in(class, levels) && haskey(highways,i)
            ...
            for j = 2:length(highway.nodes)
                if highway.nodes[first] != highway.nodes[j] && (in(highway.nodes[j], inters) || j == length(highway.nodes))
                    ...
                    dist = distancenode(nodes, route_nodes)
                   # direct = directionode(nodes,node0,node1)
                    if highway.oneway
                        s = Segment(node0, node1, route_nodes, dist, class, i, true, highway.traffic_signals,highway.maxspeed,highway.id)
                        push!(segments, s)
                    else
                        s1 = Segment(node0, node1, route_nodes, dist, class, i, false, highway.traffic_signals,highway.maxspeed,highway.id)
                        push!(segments, s1)
                        s2 = Segment(node1, node0, reverse(route_nodes), dist, class, i, false, highway.traffic_signals,highway.maxspeed,highway.id)
                        push!(segments, s2)
                    end
                    first = j
                end
...
function highwaySegments( segments::Vector{Segment} )
    highways = Dict{Int64,Highway}()
    for k = 1:length(segments)
        push!(highways,k => Highway("", 1, true, "", "", "", "$(segments[k].parent)", segments[k].nodes,highway.traffic_signals))
    end
    return highways
end
        ...
        for inter in hwy_counts[k]
            ...
            for index = 1:length(clusters)
                if issubset(intersections[inter].highways,clusters[index])
                    ...
                    c_dist = distancenode(c,nodes[inter])
                    if c_dist < max_dist
