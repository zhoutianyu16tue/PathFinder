...
### Generic Map Plot ###
# function plotMap(nodes::Dict{Int,NodeInfor};
#                  highways::Dict{Int,Highway} = nothing,
#                  bounds::@compat(Union{@compat(Void),Bounds}) = nothing,
#                  intersections::Dict{Int,Intersection} = nothing,
#                  roadways=nothing,
#                  cycleways=nothing,
#                  walkways=nothing,
...
#     xlab, ylab = if isa(nodes, Dict{Int,NodeInfor})
...
#         if km && isa(nodes, Dict{Int,NodeInfor})
...
function getNodeCoords(nodes::Dict{Int,NodeInfor}, id_list, km=false)
    coords = Array(Float64, length(id_list), 2)

    for k = 1:length(id_list)
        loc = nodes[id_list[k]].coords
        coords[k, 1] = loc.lon
        coords[k, 2] = loc.lat
    end
